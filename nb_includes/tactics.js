...
var _tactics = {
    ...
    setTarget: function (object, group) {
        ...
        if (!_math.defined(group))
            return false;
        if (_math.defined(safeGetObject(groupTargetLabel(group))))
            if (_intensity.throttled(10000, group)) // don't switch targets too often
                return false;
    ...
    unsetTarget: function (player) {
        for (let i = 0; i <= MAX_GROUPS; ++i) {
            var obj = safeGetObject(groupTargetLabel(i));
            if (_math.defined(obj) && obj.type === POSITION && _chat.findBeaconPlayer(obj.x, obj.y) === player)
                removeLabel(groupTargetLabel(i));
        }
    },
    groupDroid: function (droid) {
        if (droid.droidType === DROID_WEAPON || droid.droidType === DROID_CYBORG) {
            if (isVTOL(droid)) {
                groupAdd(vtolGroup, droid);
                return;
            }
            if (_math.withChance(100 - groupSize(miscGroup) * 50 / personality.maxMiscTanks) || firstTimeHarass) {
                firstTimeHarass = false;
                groupAdd(miscGroup, droid);
                return;
            }
            var grp = groupsBySize().filter((i) => (
                !isAlly(i) && _math.defined(findTarget(i)) && droidCanReach(droid, findTarget(i).x, findTarget(i).y)
            ));
            var ret = grp.filter((i) => (
                groupSize(i) < attackGroupSize() * 2 && _math.defined(findTarget(i))
            ));
            if (ret.length === 0)
                ret = grp;
            if (ret.length === 0)
    ...
    touchGroup: function (gr) {
        getGroupInfo(gr); // make sure everything is _math.defined
        groupInfo[gr].lastAttacked = gameTime;
    },
    fallBack: function (droid, threat) {
        if (droid.order === DORDER_MOVE)
            return;
        if (_stats.guessDroidMicro(droid) === MICRO.MELEE)
            return;
        var x = droid.x - (baseScale / 7) * (threat.x - droid.x) / _math.distance(threat, droid);
        var y = droid.y - (baseScale / 7) * (threat.y - droid.y) / _math.distance(threat, droid);
        if (x < 1 || y < 1 || x > mapWidth - 2 || y > mapHeight - 2)
            return;
        orderDroidLoc(droid, DORDER_MOVE, x, y);
    },
    dangerLevel: function (loc) {
    ...
    pushVtols: function (object) {
        ...
        for (let i = 0; i < vtols.length; ++i)
            if (this.vtolArmed(vtols[i], 1))
                for (let j = 0; j < enemies.length; ++j)
                    if (this.vtolCanHit(vtols[i], enemies[j])) {
    ...
    inPanic: function () {
        ...
        return _intensity.cached(uncached, 10000);
    }
};
...
function findLargestGroupIn(list) {
    ...
    for (let i = 0; i < MAX_GROUPS; ++i)
        sizes[i] = 0;
    list.forEach((object) => {
        if (!_math.defined(object.group) || object.group < 0 && object.group >= MAX_GROUPS)
            return;
        ++sizes[object.group];
    });
...
function findNearestGroup(x, y) {
    var ret = _math.naiveFindClusters(enumDroid(me).filter((droid) => (
    ...
    for (let i = 0; i < ret.clusters.length; ++i) {
        ...
        if (groupSize(gr[i]) > _tactics.attackGroupSize()) {
            var dist = _math.distance(ret.xav[i], ret.yav[i], x, y);
            if (dist < minDist) {
                minDist = dist;
                minIdx = i;
            }
    ...
    if (_math.defined(minIdx))
        return gr[minIdx];
    gr = groupsBySize();
    if (gr.length > 0)
        if (groupSize(gr[0]) > _tactics.attackGroupSize())
            return gr[0];
}
...
function targetSuitableForHarass(object) {
    ...
    return _intensity.cached(uncached, 60000, object.id);
}

function getGroupInfo(gr) {
    if (_math.defined(groupInfo[gr]))
        return groupInfo[gr];
...
function findTarget(gr) {
    ...
    getGroupInfo(gr);
    if (gameTime > groupInfo[gr].lastAttacked + 10000 && _math.defined(obj2) && obj2.type === POSITION)
        removeLabel(groupMicroTargetLabel(gr));
    if (!_math.defined(obj))
        obj = obj2;
    if (_math.defined(obj)) {
        if (gr === miscGroup) {
            // harass group should switch targets when its target gets protected, otherwise targets are permanent
            if (_intensity.throttled(10000) || targetSuitableForHarass(obj))
                return obj;
        } else
            return obj;
    }
    ...
    if (gr === miscGroup) {
        var list = _stats.enumStructList(miscTargets, enumLivingPlayers().filter(isEnemy).random());
        ...
        list = list.filter(targetSuitableForHarass).filter((feature) => (
            (_stats.iHaveHover() && canReachFromBase(getPropulsionStatsComponents(PROPULSIONUSAGE.HOVER)[0], feature)) ||
        ...
        obj = list[_math.random(Math.min(3, list.length))];
        if (obj) {
    ...
    var list = _stats.enumStructList(targets, gr);
    if (list.length > 0)
        obj = list.random();
    else {
...
function groupInDanger(gr) {
    getGroupInfo(gr); // make sure everything is _math.defined
    if (!_math.defined(groupInfo[gr].lastAttacked))
        return false;
...
function regroup(gr) {
    ...
    var size = _tactics.attackGroupSize();
    if (size < groupSize(gr) / 2)
        size = groupSize(gr) / 2;
    var ret = _math.naiveFindClusters(enumGroup(gr).filter(checkRepaired), (baseScale / 3));
    if (ret.maxCount === 0)
        return [];
    for (let i = 0; i < ret.clusters.length; ++i)
        if (i !== ret.maxIdx)
...
function weHaveRepair() {
    ...
    return _intensity.cached(uncached, 1000);
}
...
function attackTarget(droid) {
    ...
    if (droid.group !== miscGroup)
        if (!_math.defined(target) || !droidCanReach(droid, target.x, target.y)) {
            groupDroid(droid);
            return;
        }
    if (_math.defined(target))
...
function pickVtolTarget(droid) {
    function uncached() {
        function canHit(obj) {
            return _tactics.vtolCanHit(droid, obj);
        }
        var enemy = enumLivingPlayers().filter(isEnemy).random();
        var list;
        list = _stats.enumStructList(miscTargets, enemy).filter(canHit);
        ...
        if (list.length > 0) return list.random();
        list = _stats.enumStructList(targets, enemy).filter(canHit);
        if (list.length > 0) return list.random();
        list = enumDroid(enemy).filter(canHit);
        if (list.length > 0) return list.random();
    }
    return _intensity.cached(uncached, 100, droid.canHitAir + 2 * droid.canHitGround);
}
...
function vtolReady(droid) {
    ...
    if (droid.order !== DORDER_REARM) {
        ...
        _build.buildVtols() // actually pads
    }
    return false;
}