...
Array.prototype.random = function () {
    if (this.length > 0)
        return this[_math.random(this.length)];
}
...
var _math = {
    ...
    withChance: function distance(chancePercent) {
        return 1 + this.random(100) <= chancePercent;
    },
    ...
    //distance(x,y, obj)
    //distance(obj,x,y)
    distance: function distance(obj1, obj2, obj3, obj4) {
        var x1, x2, y1, y2;
        if (this.defined(obj1.x)) {
            x1 = obj1.x;
            y1 = obj1.y;
            if (this.defined(obj2.x)) {
                x2 = obj2.x;
                y2 = obj2.y;
            } else {
                x2 = obj2;
    ...
    zeroArray: function (l) {
        var ret = [];
        for (let i = 0; i < l; ++i)
            ret[i] = 0;
        return ret;
    },
    randomUnitArray: function (l) {
        var ret = zeroArray(l);
        ret[random(l)] = 1;
        return ret;
    },
    randomItem: function (obj) {
        var ret;
        var count = 0;
        for (const i in obj)
